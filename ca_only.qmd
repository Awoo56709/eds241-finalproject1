...
<<<<<<< HEAD
![SLR Map for San Francisco](images/slr.png) *Example of the personalized SLR map shown to treated respondents (Mildenberger et al., 2024).*
=======
```{r}
#| echo: false
#| message: false
...
library(here)
knitr::include_graphics(here("images","sf_flooding.jpg"))
```
...
**We focus on California only because the study contains two experiments, and only the California sample includes *both* the map treatment and the traffic treatment. California also provides the cleanest replication of Figure 4 due to consistent geospatial data availability and a larger sample size within a single region.**

The original study conducted this analysis on coastal communities of New Jersey, Florida, and Virginia, as well. California data comes from the San Francisco Bay Area.
=======
...
We opted to conduct the replication using only data from only one state in order to explore whether study findings hold even when considering just a subset, as well as limit the scope of our analysis to allow for a more in-depth analysis of the causal method itself.

The choice of the San Francisco Bay Area was arbitrary other than for the fact that our current Master's program is located in California.
...
The 0–3 scale corresponds to: - **0 = Not at all** - **1 = Only a little** - **2 = A moderate amount** - **3 = A great deal**
...
<<<<<<< HEAD
**Interpretation:**  
**The negative coefficient on `treat_map` indicates that respondents who saw the map reported *lower* perceived personal harm. This aligns with the study’s main finding that maps reduce concern.**
...
<<<<<<< HEAD
**Above we create bins of 50 to mimic the studies use of distance away from the SLR line**

### Distinguishing the Two Analyses

- **ATE:** Measures the *overall* causal effect of map exposure.  
- **Spatial heterogeneity:** Examines *how the effect varies* with distance from the SLR boundary.

**These answer different questions where one is about average impact and the other is about the mechanism.**
=======
...
**Our California‑only plot is noisier than the pooled four state version in the paper, but the overall pattern is the same: the treatment effect is negative across nearly all distances and there is no spike in concern at the SLR boundary. This supports the study’s conclusion that the backfire effect is widespread rather than localized.** 
=======
...
**Insert comparison photo of national results*

4. **Backfire Effect**: **The negative treatment effect across most of the distances that indicates that maps may unintentionally make respondents less concerned about sea-level rise. Even respondents with homes projected to flood showed reduced perceived harm after viewing the map.**
...
**The spatial heterogeneity analysis assumes the following:
- Randomization holds within each distance.
- Treatment effects are comparable across bins.
- Distance to the SLR boundary is measured without error.
- No sorting or manipulation around the boundary due to randomization of treatment.**
**Newly ADDED**
...
The author reports no evidence of imbalance across the treatment and control groups Our California-only replication relies on the same randomization. 

**Randomization was implemented through the Qualtrics platform, ensuring that map exposure was independent of respondent characteristics. **


*SUTVA*

Respondents completed surveys individually and therefore cannot see each others maps.
Spillovers are unlikely due to the online setting. Due to this, we believe that statistical assumptions are reasonable.

**Although all treated individuals received a map, the variation in treatment was their homes location relative to the flood zone.**
...
The outcome can be a concern as a single 4-point Likert item asking how much sea level rise will harm them personally raises some issues. The paper mentions respondents spent a median time of 33 seconds on the map page which may introduce a reaction that does not internalize the information fully The lower scores may reflect reassurance (they are not in the flood zone) or disbelief in the map 

**Treatment Validity Concerns**
