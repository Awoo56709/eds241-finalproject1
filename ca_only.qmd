...
`dist_min` — distance (meters) from the SLR boundary, where negative values correspond to homes inside the projected flood zone and positive ones outside of it.
...
dist_ca_clean <- dist_ca %>% 
  ...
  # Keep observations only where the address corresponds to the minimum distance
  # (For cases with address duplicates)
  filter(dist_min == min(dist_min)) %>%
  
  # Join traffic data on addresses, and retain corresponding emails and IDS
  inner_join(
  ...
  # Join flood zone info on addresses, keeping county, tract, and flood zone info
  inner_join(
...
-   `dist_min`: Minimum distance to SLR boundary. This is our **running variable**.
